...
def CXRmodel(model_args):
  # Load pre-trained model
  model = None
  if model_args['model_type'] == 'DeiT':
    # Load DeiT-B
...
class MURADataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # Get study i
    fname = self.image_data.iloc[idx]['path']
    studyid = fname[fname.find('_p')+1:fname.find('_image')]

    # Get region
    region = ''
    if 'SHOULDER' in fname:
        region = 'shoulder'
    elif 'HUMERUS' in fname:
        region = 'humerus'
    elif 'ELBOW' in fname:
        region = 'elbow'
        ...
        region = 'hand'
    elif 'FINGER' in fname:
        region = 'finger'
    
    # Return image and label
    return img, label, studyid, region
...
class MURAPerStudyDataset(Dataset):
    def __init__(self,
                 images_file=None,  # txt file with each image name on a separate line
                 images_dir=None,  # path to dir containing images
                 image_paths_file=None,  # txt file containing image paths from images_dir
        ...
        def getStudyNameFromFileName(file_name):
            study_name = file_name[file_name.find("_")+1:file_name.rfind("_")]
            return study_name

        self.studies = set()
        for i in range(len(self.image_data)):            
            study = getStudyNameFromFileName(self.image_data.index[i])
            self.studies.add(study)
        self.studies = list(self.studies)
        random.shuffle(self.studies) # randomize order since set would maintain in order


        # Map each study to its collection of images
        self.study_to_image_path = {s: [] for s in self.studies}
        self.study_to_image_file = {s: [] for s in self.studies}
        for i in range(len(self.image_data)):
            study = getStudyNameFromFileName(self.image_data.index[i])
            self.study_to_image_path[study].append(self.image_data['path'].iloc[i])
            self.study_to_image_file[study].append(self.image_data.index[i])

        # Get max number of images in study
        self.max_images_in_study = 0
        for img_paths in self.study_to_image_path.values():
            self.max_images_in_study = max(len(img_paths), self.max_images_in_study)
    ...
    # Gets tensor vector of labels for given study name
    def getLabel(self, study_name):
        sample_image_file = self.study_to_image_file[study_name][0]
        label = torch.zeros(len(self.labels))
        if self.image_data.loc[sample_image_file, 'Positive'] == 1:
            label[1] = 1
        else:
            label[0] = 1
        return label
    ...
    def __getitem__(self, idx):
        ...
        # Get images in study
        image_paths = self.study_to_image_path[study]
        ...
        # Pad images array to make length same as max number of images in study
        num = len(images)
        for _ in range(num, self.max_images_in_study):
            images.append(images[0])

        # Convert images to a tensor (concatenate along new dimension)
        images = torch.stack(images, dim=0)

        # Get label
        label = self.getLabel(study)

        # Store number of images in study
        num = torch.tensor([num])

        # Return image and label
        return images, label, num
