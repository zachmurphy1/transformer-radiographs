...
for var in ['auc_weighted',
            'precision_micro','recall_micro','f1_micro','accuracy_micro'] + ['{}_{}'.format(m,l) for m in ['auc','precision','recall','f1','accuracy'] for l in regions]:
...
pc = pd.DataFrame(pc)
pd.DataFrame(pc).to_csv(os.path.join(args.dir_name, 'ttest_table.csv'),index=False)
...
if args.plots == 'y':
  ...
  for d in dataset:
    ...
    for i, l in enumerate([x.replace(' ', '_') for x in labels_sub]):
      # Get p value
      pc_ = pc[pc['Metric']=='{}_{}'.format(metric,l)]
      p = pc_['p'].tolist()[0]
