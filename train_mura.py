...
Output:
  ...
  run stats log: prints stats to [results_dir]/mura_results.csv
...
parser.add_argument("--pretrained", default='y', type=str, help='se pretrained model (y/n)')
parser.add_argument("--labels-set", default='mura', type=str, help='labels set to use in cfg')
...
parser.add_argument("--break-patience", default=5, type=int, help='umber of epochs to wait for improvement before stopping training')
parser.add_argument("--dataset", default='mura', type=str, help='selects dataset (nihcxr14/chexpert/padchest/mimic)')
...
parser.add_argument("--num-workers", default=12, type=int, help='umber of dataloader workers')
...
parser.add_argument("--results-dir", default='/export/gaon1/data/zmurphy/transformer-cxr/mura_results', type=str, help='directory to save results')
parser.add_argument("--copy-to-local", default='n', type=str, help='***')
...
train_data = MURA.MURADataset(images_file=os.path.join(dataset_root, model_args['train_file']),
...
val_data = MURA.MURADataset(images_file=os.path.join(dataset_root, model_args['val_file']),
                            images_dir=os.path.join(dataset_root, 'images'),
                            image_paths_file=os.path.join(dataset_root, 'image_paths.txt'),
                            labels_file=os.path.join(dataset_root, 'labels.csv'),
                            labels=model_args['labels'],
...
for e in range(model_args['max_epochs']):
    ...
    for b in range(len(trainLoader)):
        epoch_points.append(MURA.getCutmixPoints(mixup_settings))
...
for epoch in range(model_args['max_epochs']):
    ...
    for x, y in trainLoader:
        ...
        if model_args['use_mixup']:
            if x.shape[0] % 2 != 0:
                x = x[:-1, :, :, :]
                y = y[:-1, :]
            x, y = MURA.mixup(x, y, mixup_settings=mixup_settings, epoch=epoch, batch_num=batch_counter - 1,
                              device=device)
            yhat = model(x)
    ...
    for x, y, idxs in valLoader:
        ...
        with torch.no_grad():
            ...
            val_studies.extend([val_data.getStudyName(idx[0]) for idx in idxs.to('cpu').numpy().tolist()])
    ...
    # Compute and record auc for validation data on a study level
    val_df = pd.DataFrame(columns=['study', 'y[0]', 'y[1]', 'yhat[0]', 'yhat[1]'])
    val_df['study'] = val_studies
    for i in range(len(val_ys)):
        val_df['y[0]'].at[i] = val_ys[i][0]
        val_df['y[1]'].at[i] = val_ys[i][1]
        val_df['yhat[0]'].at[i] = val_yhats[i][0]
        val_df['yhat[1]'].at[i] = val_yhats[i][1]
    val_df[['y[0]', 'y[1]', 'yhat[0]', 'yhat[1]']] = val_df[['y[0]', 'y[1]', 'yhat[0]', 'yhat[1]']].apply(pd.to_numeric)
    val_df_study = val_df.groupby('study').mean()
    val_ys_study = []
    val_yhats_study = []
    for i in range(len(val_df)):
        val_ys_study.append([val_df['y[0]'].iloc[i], val_df['y[1]'].iloc[i]])
        val_yhats_study.append([val_df['yhat[0]'].iloc[i], val_df['yhat[1]'].iloc[i]])
    train_log['auc']['val'].append(roc_auc_score(val_ys_study, val_yhats_study, average='weighted'))
    ...
    # Print
    print(
        'Epoch {}\tTrain loss: {:.4f} Val loss: {:.4f} Train auc: {:.4f} Val auc: {:.4f} Time (min): {:.2f} Total time: {:.2f}'.format(
            epoch,
...
printToResults(','.join([str(x) for x in results.values()]) + '\n',
               os.path.join(model_args['results_dir'], 'mura_results.csv'))
